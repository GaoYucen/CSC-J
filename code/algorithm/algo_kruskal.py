...
import clustering.kruskal_clustering as ks
import time
...
def run(points, M):
    ...
    for route_num in range (1,M+1):
        ...
        for _ in range(Iteration):
            ...
            #%% 高维空间聚类+划分簇
            index = ks.clustering(points_p, route_num)[0]
    ...
    return min(best_tsp_value_route), np.argmin(best_tsp_value_route) + 1, best_tsp_value_route
...
if __name__ == "__main__":
    ...
    min_val, best_k, all_vals = run(points, M)
    print(f"Min: {min_val}, Best K: {best_k}")
